...
void TotalFileReverse(string filename, size_t chunkSize,int st, int end,string flagVal) {
    try {
        ...
        //End check directory created 

        

        //start code to create new file 
        string newFileName = "Assignment1/"+flagVal+"_"+filename;
        int fd2;
        
         fd2= open(newFileName.c_str(),O_WRONLY| O_CREAT,0600);
...
void readFileInReverseChunks(string filename, size_t chunkSize,int st, int end, int doReverse,string flagVal) {
    try {
 ...
 //start check directory created .
        ...
        //End check directory created 
           ...
           //start code to create new file 
        string newFileName = "Assignment1/"+flagVal+"_"+filename;
        int fd2 = open(newFileName.c_str(),O_WRONLY| O_CREAT|O_APPEND,0600);
         if(fd2==-1){
            throw runtime_error("Error in creating/opening new file");
...
int main(int argc, char* argv[]){
    ...
    if(flag=="0"){
        ...
        readFileInReverseChunks(fileName,chunkSize,0,-2,true,flag);
    }else if(flag=="1"){
         if(argc!=3){
            printError("Incorrect Input params");
            return 0;
        }
        TotalFileReverse(fileName,chunkSize,0,-2,flag);
        ...
        if(st>end){
            ...
            TotalFileReverse(fileName,chunkSize,0,st,flag);
            readFileInReverseChunks( fileName,chunkSize,st,end+1,false,flag);
            TotalFileReverse(fileName,chunkSize,end+1,-2,flag);
    }
